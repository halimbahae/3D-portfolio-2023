...
let projects = [
  {
    image: 'textures/project08.jpg',
    url: 'https://drive.google.com/file/d/1k672V6z3yeJ5hXjVo2CaHmCWEEo4BEa_/view?usp=sharing',
  },
  {
    image: 'textures/youtube.png',
    url: 'https://youtu.be/MCav70hTXKw',
  },
...
function loadIntroText() {
  ...
  loader.load('fonts/helvatica.json', function (font) {
    ...
    const titleGeo = new TextGeometry('BAHAE  EDDINE', {
      font: font,
      size: 0.07,
      height: 0.01,
    });
...
// elevator-pitch menu
document.getElementById('elevator-pitch-btn').addEventListener('click', (e) => {
  e.preventDefault();
  document
    .querySelector('.elevator-pitch__dropdown')
    .classList.toggle('elevator-pitch__dropdown--open');
});

document.addEventListener('mouseup', (e) => {
  const container = document.querySelector('.elevator-pitch-menu');
  if (!container.contains(e.target)) {
    container
      .querySelector('.elevator-pitch__dropdown')
      .classList.remove('elevator-pitch__dropdown--open');
  }
});
